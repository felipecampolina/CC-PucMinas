...
int editDistRec(TreeNode* t1, TreeNode* t2) {
    // Se a primeira árvore está vazia, a única opção é inserir todos os nós da segunda árvore
    if (!t1) return t2 ? 1 + editDistRec(nullptr, t2->children.empty() ? nullptr : t2->children[0]) : 0;

    // Se a segunda árvore está vazia, a única opção é remover todos os nós da primeira árvore
    if (!t2) return t1 ? 1 + editDistRec(t1->children.empty() ? nullptr : t1->children[0], nullptr) : 0;

    // Custo de editar o nó raiz (substituição se os rótulos são diferentes)
    int cost = (t1->label == t2->label) ? 0 : 1;

    // Obter tamanhos dos filhos
    int m = t1->children.size();
    int n = t2->children.size();

    // Se nenhuma das árvores tem filhos, o custo é apenas o custo de editar o nó raiz
    if (m == 0 && n == 0) {
        return cost;
    }

    // Se a primeira árvore não tem filhos, a única opção é inserir todos os filhos da segunda árvore
    if (m == 0) {
        int insertCost = 0;
    ...
    // Se a segunda árvore não tem filhos, a única opção é remover todos os filhos da primeira árvore
    if (n == 0) {
        int removeCost = 0;
    ...
    // Se ambas as árvores têm filhos, considerar a distância de edição entre todas as combinações de filhos
    int minCost = INT_MAX;
    for (int i = 0; i < m; ++i) {
        for (int j = 0; j < n; ++j) {
            ...
            for (int k = 0; k < m; ++k) {
                if (k != i) {
                    currentCost += 1 + editDistRec(t1->children[k], nullptr); // Remover
                }
            }
            for (int l = 0; l < n; ++l) {
                if (l != j) {
                    currentCost += 1 + editDistRec(nullptr, t2->children[l]); // Inserir
                }
            }
...
int main() {
    vector<int> sizes = {100, 200, 300, 400, 500, 5000, 6000};
    for (int size : sizes) {
