...
public class OrdenacaoExternaNumeros extends CRUD {
  int N = 5000; // size of the file in disk
  int M = 5000; // max items the memory buffer can hold

  OrdenacaoExternaNumeros() {
    this.N = 0;
    this.M = 0;

  }

  OrdenacaoExternaNumeros(int n, int m) {
    this.N = n;
    this.M = m;
...
class noMinHeap {
  ...
  public noMinHeap(int elemento, int i) {
    this.elemento = elemento;
    this.i = i;
  }

  public void swap(noMinHeap x, noMinHeap y) {
    noMinHeap temp = x;
    x = y;
    y = temp;
  }
...
class MinHeap {
  ...
  public noMinHeap[] swap(noMinHeap x, noMinHeap y) {
    noMinHeap resultado[] = new noMinHeap[2];

    noMinHeap temp = new noMinHeap(x.elemento, x.i);
    x = new noMinHeap(y.elemento, y.i);
    y = new noMinHeap(temp.elemento, temp.i);
    resultado[0] = x;
    resultado[1] = y;
    return resultado;
  ...
  public void MinHeapify(int i) {
    ...
    if (l < heap_size && harr[l].elemento < harr[i].elemento)
      smallest = l;

    if (r < heap_size
        && harr[r].elemento < harr[smallest].elemento)
      smallest = r;

    if (smallest != i) {

      noMinHeap temp = new noMinHeap(harr[i].elemento, harr[i].i);
      harr[i] = new noMinHeap(harr[smallest].elemento, harr[smallest].i);
      harr[smallest] = new noMinHeap(temp.elemento, temp.i);

      MinHeapify(smallest);
    }
  }

  public static BufferedReader openFile(String fileName, String mode) throws FileNotFoundException {
    File file = new File(fileName);
    FileReader x = new FileReader(file);
    BufferedReader fp = new BufferedReader(x);
    return fp;
  }

  public static BufferedWriter criarArquivo(String fileName) throws IOException {
    File file = new File(fileName);
    FileWriter x = new FileWriter(file);
    BufferedWriter fp = new BufferedWriter(x);
    return fp;
  }

  public static void mergeFiles(String output_file, int n, int k) throws IOException {

    BufferedReader in[] = new BufferedReader[k];

    for (int i = 0; i < k; i++) {
      String tfile = "arquivosTemporarios/temp-file-" + i + ".txt";

      // convert i to string
      // snprintf(fileName, fileName.length(), "%d", i);

      // Open output files in read mode.
      in[i] = openFile(tfile, "wr");
    }

    // FINAL OUTPUT FILE
    BufferedWriter out = criarArquivo(output_file);
    ...
    for (i = 0; i < k; i++) {
      ...
      if (!in[i].ready())
        break;
      String x = in[i].readLine();

      harr[i] = new noMinHeap(Integer.parseInt(x), i);
    ...
    while (count != i) {
      ...
      noMinHeap root = new noMinHeap(0, i);
      root = hp.getMin();
      /// System.out.println("hp.elemento = "+hp.harr[0].elemento);
      // System.out.println("hp.i"+hp.harr[0].i);

      // System.out.println("root.elemento = "+root.elemento);
      // System.out.println("root.i"+root.i);
      out.write(Integer.toString(root.elemento));
      out.newLine();

      if (!in[root.i].ready()) {
        root.elemento = Integer.MAX_VALUE;
        // System.out.println("ENTREI");
        count++;
      } else {
        root.elemento = Integer.parseInt(in[root.i].readLine());
      }
  ...
  public static void createInitialRuns(String input_file, int run_size,
      int num_ways) throws IOException {
    // For big input file
    BufferedReader in = openFile(input_file, "rw");

    // output scratch files
    BufferedWriter out[] = new BufferedWriter[num_ways];

    for (int i = 0; i < num_ways; i++) {
      String fileName = "arquivosTemporarios/temp-file-" + i + ".txt";

      // Open output files in write mode.
      out[i] = criarArquivo(fileName);
    }
    ...
    while (more_input) {
      ...
      for (int j = 0; j < i; j++) {
        out[next_output_file].write(Integer.toString(arr[j]));
        out[next_output_file].newLine();
  ...
  public static void externalSort(String input_file, String output_file,
      int num_ways, int run_size) throws IOException {
    // read the input file,
  ...
  public static void main(String[] args) throws IOException {
    ...
    String input_file = "dados/input2.txt";
    String output_file = "dados/output.txt";
