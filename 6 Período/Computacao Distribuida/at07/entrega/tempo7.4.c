...
int main(int argc, char *argv[]) {
    ...
    while (time() < 150.0) { /* Executa até o tempo 150 */
        ...
        switch (evento) {
                ...
                for (i = 0; i < n; i++) {
                    if (i != teste) { /* O processo não testa ele mesmo */
                        ...
                        if (status_proc == 0) {
                            ...
                            /* Coleta o estado do processo testado */
                            for (j = 0; j < n; j++) {
                                if (j != teste && proc[teste].State[j] == -1 && proc[i].State[j] != -1) {
                                    /* Atualiza o estado do processo testador com as informações coletadas */
                                    proc[teste].State[j] = proc[i].State[j];
                                    printf("O processo %d coletou o estado do processo %d a partir do processo %d no tempo %.1f\n", teste, j, i, time());
                                }
                            }
            ...
            case fault:
                /* Processa a falha */
                if (request(proc[teste].id, teste, 0.0) != 0) {
