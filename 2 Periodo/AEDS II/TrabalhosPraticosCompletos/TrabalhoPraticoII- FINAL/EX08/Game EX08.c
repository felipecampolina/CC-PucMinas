...
int main(void)
{
    ...
    for(int g = 0 ; g<=tamanho; g++) {
        ...
        comando[1] = input2[1];
        if(comando[1]!='*'){
        for(int i = 3, j=0 ; i<strlen(input2);i++,j++){
            idComando[j] = input2[i];
        }
        faztudo(comando,atoi(idComando));
        }
        else{
            int k = 3;
            int x = 0;
        ...
        comando[1] = input2[1];
            ...
            for(int i = k+1, j=0 ; i<strlen(input2);i++,j++){
            idComando[j] = input2[i];
        }
                     
        faztudoAsterisco(comando,atoi(idComando),atoi(posicao));
        }
        if (input2[strlen(input2) - 2] < ' ') input2[strlen(input2) - 2] = '\0';
        else if (input2[strlen(input2) - 1] < ' ') input2[strlen(input2) - 1] = '\0';
    }
    for(int i = 0 ; i<n;i++){
         ids[games_sz++] = array[i];
    }
if (games_sz > 0) {
        // Calls function to read from file and populate array of games
        game_read_csv(games, ids, games_sz);
