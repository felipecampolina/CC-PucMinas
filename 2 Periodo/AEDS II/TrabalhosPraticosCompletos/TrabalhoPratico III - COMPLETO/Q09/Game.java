...
public class Game {//Inicio classe Game
 ...
 //---Fim atributos---//
  ...
  //---Fim Setters e Getters---//
    ...
    public float juntarAtributos(int listaSeparada,String path) throws IOException{//Inico juntarAtributos -- funcao que junta os atribytos a partir do id da lista
        BufferedReader leitura = new BufferedReader(new FileReader(path));
        String frase = leitura.readLine();
    ...
    public void sort(float array[]) {
      ...
      */
     private void mergesort(int esq, int dir,float array[]) {
        if (esq < dir){
           int meio = (esq + dir) / 2;
      ...
      * @param int dir fim do array a ser ordenado
      */ 
     public void intercalar(int esq, int meio, int dir,float array[]){
        int n1, n2, i, j, k;
        ...
        for(i = j = 0, k = esq; k <= dir; k++){
         ...
         public String juntarAtributos2(int listaSeparada,String path,Lista lista) throws IOException{//Inico juntarAtributos -- funcao que junta os atribytos a partir do id da lista
            BufferedReader leitura = new BufferedReader(new FileReader(path));
            String frase = leitura.readLine();
            String teste[] = new String[500];
            teste = frase.split(",");
        ...
        public String achaFraseCerta (String frases[],float nomesJogosListaInteiros,Game Game,int idAntigo){//Inicio achaFraseCerta -- funcao que compara todas as frases e os upvotes retornando a farse certa para cada game
            String frase;
            int i = 0;
            Game = new Game();
            Game.ler(frases[i]);
            while(!(nomesJogosListaInteiros==Game.getUpvotes()) || !(Game.getAppId()!=idAntigo)){
                Game = new Game();
                i++;
                Game.ler(frases[i]);
        ...
        public static void main(String[] args)  throws Exception {//Inicio main
            ...
            while(!Game.isFim(id)){ //lendo e criando a lista
                ...
                lista.inserirFim(Game.getAppId());//insere ids na lista
                id = MyIO.readLine();
            }
            float nomesJogosListaInteiros[] = new float[lista.getTamanho()];
            String frases[] = new String[lista.getTamanho()];
        
            for(int i = 0 ; i<lista.getTamanho();i++){
                nomesJogosListaInteiros[i] = Game.juntarAtributos(lista.getElemento(i), path);//atribui array de float para cada upvote
                frases[i]=Game.juntarAtributos2(lista.getElemento(i), path,lista);//atribue array de string para cada frase
            }
            Game.sort(nomesJogosListaInteiros);//Ordena upvotes usando MergeSort
            ...
            for(int i = 0 ; i<lista.getTamanho();i++){
                Game = new Game();
                fraseCerta = Game.achaFraseCerta(frases, nomesJogosListaInteiros[i], Game,idAntigo);//acha frase certa 
                Game.ler(fraseCerta);//Le game
                idAntigo = Game.getAppId();//pega o id antigo para previnir repeticoes
                Game.imprimir();//Imprime
            }
            
          // --- Criando arquivo log --- //
          FileWriter arq = new FileWriter("matricula_mergesort.txt");
          PrintWriter gravarArq = new PrintWriter(arq);
          gravarArq.printf("Matricula: 762732 - MergeSort -  Q09\t");
          gravarArq.printf("N de Comparações: n*log(n)\t");
          gravarArq.printf("N de Movimentos: n*log(n)) \t");
