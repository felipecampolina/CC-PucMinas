...
public class Game {//Inicio classe Game
 ...
 //---Fim atributos---//
  ...
  //---Fim Setters e Getters---//
    ...
    public String juntarAtributos(int listaSeparada,String path) throws IOException{//Inico juntarAtributos -- funcao que recebe o id e retorna a frase
        BufferedReader leitura = new BufferedReader(new FileReader(path));
        String frase = leitura.readLine();
    ...
    public void swap(int i, int j,Date array[]) {
     ...
     public Date[] sort(Date array[],Lista lista) {
        //Alterar o vetor ignorando a posicao zero
        int n = lista.getTamanho();
        Date[] tmp = new Date[n+1];
        for(int i = 0; i < n; i++){
           tmp[i+1] = array[i];
        }
        array = tmp;
  ...
  //Contrucao do heap
     public void construir(int tamHeap,Date array[]){
        for(int i = tamHeap; i > 1 && array[i].compareTo(array[i/2])>0; i /= 2){
           swap(i, i/2,array);
        }
     }
  
  //Contrucao do heap
     public void reconstruir(int tamHeap,Date array[]){
        int i = 1;
        while(i <= (tamHeap/2)){
     ...
     public int getMaiorFilho(int i, int tamHeap,Date array[]){
        int filho;
        if (2*i == tamHeap || array[2*i].compareTo(array[2*i+1])>0){
           filho = 2*i;
        } else {
           filho = 2*i + 1;
        }
        return filho;
            ...
            public boolean pesquisarEmArray(int array[],int x){//Inicio pesquisar em Array -- funcao que pesuisa se o int x esta no array de inteiros
                boolean retorno = false;
        ...
        public String achaFraseCerta (String frases[],Date nomesJogosListaInteiros,Game Game){//Inicio achafraseCerta-- funcao que recebe uma string de frases do arquivo csv e retorna a farse certa comparando com uma data especifica
            String frase;
            int i = 0;
            Game = new Game();
            Game.ler(frases[i]);
        ...
        public static void main(String[] args)  throws Exception {//Inicio main
            ...
            Date nomesJogosListaInteiros[] = new Date[lista.getTamanho()];
            String frases[] = new String[lista.getTamanho()];

            for(int i = 0 ; i<lista.getTamanho(); i++){
                    frases[i]=Game.juntarAtributos(lista.getElemento(i), path); // cria um array com todas frases dos ids do verde
                    Game.ler(frases[i]);// Le e atribue as frases do id do verde para logo em seguida pegar seus atrbutos
                    nomesJogosListaInteiros[i] = Game.getReleaseDate();//cria um array de datas com as datas dos games digitados pelo pub in
            }

            nomesJogosListaInteiros= Game.sort(nomesJogosListaInteiros,lista);//ordena usando heapsort

            for(int i = 1 ; i<lista.getTamanho()+1; i++){
                Game = new Game();
                String fraseCerta ="";
                fraseCerta = Game.achaFraseCerta(frases,nomesJogosListaInteiros[i],Game);//Atribue a uma string a frase certa para ser lida e escrita posteriormente
               ...
               gravarArq.printf("Matricula: 762732 - HEAPSORT -  Q05\t");
               gravarArq.printf("N de Comparações: lg(n)  \t");
               gravarArq.printf("N de Movimentos: n * lg(n) \t");
