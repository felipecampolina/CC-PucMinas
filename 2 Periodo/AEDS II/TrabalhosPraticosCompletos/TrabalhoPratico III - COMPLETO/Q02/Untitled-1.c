...
char* game_printOnlyName(game *g) 
{
    char* resposta;
    resposta = g->name.buf;
    return resposta;
...
char* imprimeNomeJogoPeloID(int id){
...
if (games_sz > 0) {
        ...
        // Prints the games read and frees the heap allocated memory
           resposta = game_printOnlyName(games);
...
void faztudo(char comando[],int idComando){
    if(comando[0]=='R'&&comando[1]=='I'){
        imprimeNomeJogoPeloID(removerInicio());

    }
    else if(comando[0]=='R'&&comando[1]=='F'){
        imprimeNomeJogoPeloID(removerFim());

    }
    else if(comando[0]=='I'&&comando[1]=='I'){
        inserirInicio(idComando);

    }
    else if(comando[0]=='I'&&comando[1]=='F'){
        inserirFim(idComando);

    }

}
void faztudoAsterisco(char comando[],int idComando,int posicao){
    if(comando[0]=='I'){
        inserir(idComando,posicao);
    }
    else{
...
void swap(int menor, int i,int array[]) {//Inicio swap -- troca array de posicoes
   int temp = array[menor];
   array[menor] = array[i];
   array [i] = temp;
}//Fim swap
void selecao(int array[], int n){//Inicio selecao -- Ordena por selecao
...
char* respostadoJ;
    ...
    for (int i = 0; i < (n - 1); i++) {
      int menor = i;
      respostaMenor =imprimeNomeJogoPeloID(array[i]);
      for (int j = (i + 1); j < n; j++){
        respostadoJ = imprimeNomeJogoPeloID(array[j]);
         if (strcmp(respostadoJ,respostaMenor)<0){
             menor = j;
          }
      }
      swap(menor, i,array);
...
void bolha(int array[], int n){//Inicio bolha
...
char* respostadoJ;
    ...
    for (i = (n - 1); i > 0; i--) {
      for (j = 0; j < i; j++) {
        respostaMenor =imprimeNomeJogoPeloID(array[j]);
        respostadoJ = imprimeNomeJogoPeloID(array[j+1]);
         if (strcmp(respostadoJ,respostaMenor)<0) {
            swap(j, j+1,array);
         }
      }
...
int main(void)
{
    ...
    inserirInicio(atoi(input));
    input[strlen(input) - 1] = '\0';
    while (!is_fim(input))  {
        fgets(input, MAX_LINE_SIZE, stdin);
        inserirFim(atoi(input));
        // checking if is a reserved char to then trim it
    ...
    int idOrdenado[MAX_FIELD_SIZE];

 for(int i = 0 ; i<n;i++){
     idOrdenado[i] = array[i];
    }


    selecao(idOrdenado,n);//Ordena array
    bolha(idOrdenado,n);//Ordena array
    

    
    for(int i = 0 ; i<n;i++){
        //printf("%i\n",idOrdenado[i]);
         ids[games_sz++] = idOrdenado[i];//Atribue ids usando array ordenado
...
if (games_sz > 0) {
  ...
  fprintf(arq,"Matricula: 762732 - SELECAORECURSIVA\t");
   fprintf(arq,"Comparacoes: (n^2-n)/2\t");
   fprintf(arq,"Movimentos: n^2\t");
   fprintf(arq,"Execucao: %d s",tempo);
