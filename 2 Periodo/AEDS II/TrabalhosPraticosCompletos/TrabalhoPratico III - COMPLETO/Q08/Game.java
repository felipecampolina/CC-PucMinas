...
public class Game {//Inicio classe Game
 ...
 //---Fim atributos---//
  ...
  //---Fim Setters e Getters---//
    ...
    public void swap(int j, int i , String array[]){
        String temp = array[j];
        array[j]=array[i];
        array[i] = temp;
    }

    public void bolha(String array[],int n) {
		for (int i = (n - 1); i > 0; i--) {
			for (int j = 0; j < i; j++) {
        ...
        public String juntarAtributos3(String listaSeparada,String path) throws IOException{//Inico juntarAtributos -- funcao que junta os atribytos a partir do id da lista
            BufferedReader leitura = new BufferedReader(new FileReader(path));
            String frase = leitura.readLine();
            String teste[] = new String[500];
            //teste = frase.split(",");
    
            while(!(listaSeparada.contentEquals(getDevelopers())) && !(isFim(frase))){ //teste[0] = id
                frase = leitura.readLine();
                //teste = frase.split(",");
            }
            return frase;
        ...
        public boolean pesquisarEmArray(int array[],int x){//inicio pesquisaArray -- funcao que recebe um inteiro e um array de inteiros e confere se x esta presente no array
            boolean retorno = false;
        ...
        public static void main(String[] args)  throws Exception {//Inicio main
            ...
            for(int i = 0 ; i<lista.getTamanho();i++){
                nomesJogosListaInteiros[i] = Game.juntarAtributos(lista.getElemento(i), path);//Atribue o id de cada game para um array
            }
            ...
            for(int i = 0 ; i<lista.getTamanho();i++){
                Game Game3 = new Game();
                nomesJogosLista[i]=Game.juntarAtributos2(nomesJogosListaInteiros[i], path);//Atribue a frase do csv para cada id do verde 
                Game3.ler(nomesJogosLista[i]);//Le game
                nomesJogosLista[i]=Game3.getDevelopers();// Sobrescreve array antigo com os desenvolvedores de cada jogo
            }
            Game.bolha(nomesJogosLista,nomesJogosLista.length);//Ordena desenvolvedores pelo bolha



            //-- Processo que confere se o jogo possui o MESMO desenvolvedor e  MESMO id , caso positivo irá imprimir 
            for(int i = 0 ; i<lista.getTamanho();i++){
                Game Game4 = new Game();
                BufferedReader leitura = new BufferedReader(new FileReader(path));
                String frase = leitura.readLine();
            ...
            while(!(nomesJogosLista[i].contentEquals(Game4.getDevelopers())) && !Game4.isFim(frase) || continuar ){ //teste[0] = id
                continuar = false;
                Game4 = new Game();
                frase = leitura.readLine();
                Game4.ler(frase);
                if(!Game4.pesquisarEmArray(nomesJogosListaInteiros,Game4.getAppId())) continuar = true;
           ...
           gravarArq.printf("Matricula: 762732 - BOLHA -  Q06\t");
           gravarArq.printf("N de Comparações: n(n-1)/2\t");
