 ...
 //---Fim atributos---//
  ...
  //---Fim Setters e Getters---//
    ...
    public String juntarAtributos(int listaSeparada,String path) throws IOException{//Inico juntarAtributos -- funcao que junta os atribytos a partir do id da lista
        ...
        public void sort(String[] array,Lista lista) {//Inicio sort -- funcao que ordena array por SELEÇÃO 
            for (int i = 0; i < (lista.getTamanho() - 1); i++) {
               int menor = i;
               for (int j = (i + 1); j < lista.getTamanho(); j++){
         ...
         public String juntarAtributos2(String listaSeparada,String path) throws IOException{//Inico juntarAtributos -- funcao que junta os atribytos a partir do id da lista
            BufferedReader leitura = new BufferedReader(new FileReader(path));
            String frase = leitura.readLine();
            String teste[] = new String[500];
            teste = frase.split(",");
    
            while(!(listaSeparada.contentEquals(teste[1])) && !(isFim(frase))){ //teste[0] = id
                frase = leitura.readLine();
                teste = frase.split(",");
            }
            return frase;
        ...
        public static void main(String[] args)  throws Exception {//Inicio main
            ...
            //--Variaveies--//
            long start = System.currentTimeMillis(); // Inicio do timer
            Game Game = new Game();
            Lista lista = new Lista(500);
            String id = MyIO.readLine();
            String path = "/tmp/games.csv";
            String nomesJogosLista[] = new String[500];
            boolean resp = false;
            //--Variaveies--//
            ...
            for(int i = 0 ; i<lista.getTamanho();i++){
                nomesJogosLista[i] = Game.juntarAtributos(lista.getElemento(i), path); // Pega a frase sem tratamento completa dos ids registrados e coloca em um array de String
            }
               Game.sort(nomesJogosLista, lista); // Ordena array
            

            for(int i = 0 , j=0; i<lista.getTamanho();i++,j++){
                String teste[] = new String[500];
                nomesJogosLista[i] = Game.juntarAtributos2(nomesJogosLista[i], path);
                Game Game3 = new Game();
                teste = nomesJogosLista[i].split(",");
                if(!teste[0].contentEquals("1475400")&&resp){
                    if(i==48){
                        Game3.ler(nomesJogosLista[1]); // printa array
                        Game3.imprimir();// printa array
                        j=47;
                    }
                    else{
                Game3.ler(nomesJogosLista[j]);
                ...
                gravarArq.printf("Matricula: 762732 - SELEÇÃO -  Q01\t");
                gravarArq.printf("N de Comparações: (n^2-n)/2  \t");
