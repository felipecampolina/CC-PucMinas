//EX 08 TP 02 - Felipe Campolina Soares de Paula -- 762732
...
void game_printOnlyName(game *g) 
{
    printf( "%s\n",g->name.buf);
}
...
void imprimeNomeJogoPeloID(int id){
...
void faztudo(char comando[],int idComando){//Inicio fazTudo - funcao que espeficia o comando e assim remove ou insere um id na lista
   if(comando[0]=='R'){
    imprimeNomeJogoPeloID(remover(idComando));
   }
   else if (comando[0]=='I'){
        inserir(idComando);
    }
...
void faztudoAsterisco(char comando[],int idComando,int posicao){//Inicio faztudoAsterisco - - funcao que especifica o comando e assim remove ou insere um id na lista, considerando o *
    if(comando[0]=='I'){
        inserir(idComando);
    }
    else{
       imprimeNomeJogoPeloID(remover(posicao));
    }

}//FIm faztudoAsterisco 
...
int main(void)
{
    ...
    char input2[MAX_FIELD_SIZE];
    strarr input2Separado[MAX_FIELD_SIZE];
    //new_strarr(input2Separado,MAX_FIELD_SIZE);
     char comando[MAX_FIELD_SIZE];
     char idComando[MAX_FIELD_SIZE];
    char posicao[MAX_FIELD_SIZE];
    int array[MAX_FIELD_SIZE];

    
    int tamanho = 0;
    scanf("%i",&tamanho);//Pega numero inteiro de comandos a ser executado

    for(int g = 0 ; g<=tamanho; g++) {//repete n vezes
        fgets(input2, MAX_LINE_SIZE, stdin);
        comando[0] = input2[0];// comando = R ou I
        if(comando[1]!='*'){// Se não for *
        for(int i = 2, j=0 ; i<strlen(input2);i++,j++){
            idComando[j] = input2[i]; // Pega id do comando 
        }
        faztudo(comando,atoi(idComando)); // chama faz tudo , mandando um comando e o ID(inteiro)
        }
        if (input2[strlen(input2) - 2] < ' ') input2[strlen(input2) - 2] = '\0'; // Sentinela do vetor
        else if (input2[strlen(input2) - 1] < ' ') input2[strlen(input2) - 1] = '\0'; // Sentinela do vetor
    }
    ...
    for(i = topo; i != NULL; i = i->prox) {
      ...
      array[g] =  i->elemento;
      //printf("%i\n",array[g]);
      ids[games_sz++] = array[g]; // Cria um array de ids para chama funcao de baixo
      g++;
   }

    size_t idsInvertido[MAX_GAMES];
    int j = n-1;

    for(int i = 0 ; i<n ; i++,j--){
        idsInvertido[i] = ids[j];
    }
    ...
    if (games_sz > 0) {
        // Calls function to read from file and populate array of games
       game_read_csv(games, idsInvertido, games_sz); // Chama funcao principal para que os tratar dados e printar enunciado
       //printf("%i\n",ids[0]);
        ...
        for (int i = 0; i < n; i++) {
            printf("[%i]",i);
            game_print(games + i);//Printa lista 
            //game_delete(games + i);
        }
