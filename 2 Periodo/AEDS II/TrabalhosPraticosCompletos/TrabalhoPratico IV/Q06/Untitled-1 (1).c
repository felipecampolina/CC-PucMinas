...
#define FILE_PATH "/tmp/games.csv"//TA AQUI O CARALHO DO CAMNINHO
...
int game_printOnlyName(game *g) 
{
    int resposta;
    resposta = g->release_date.value;
    return resposta;
}

int game_printOnlyId(game *g) 
{
    int resposta;
    resposta = g->app_id;
    return resposta;
...
int imprimeValueJogoPeloID(int id){
    int n = tamanho();
    int resposta;
...
int substitueID(int data, int idParDataImpar[]){
    int resp;
    int i = 0;
    while(data != idParDataImpar[i]){
        i++;
    }
    resp = idParDataImpar[i-1];
}
...
int main(void)
{
    ...
    int datasEmSegundos[MAX_FIELD_SIZE];
    int idParDataImpar[MAX_FIELD_SIZE];
    int g = 0;

   for(i = primeiro->prox; i != NULL; i = i->prox) {
      array[g] = i->elemento;
      datasEmSegundos[g] = imprimeValueJogoPeloID(array[g]);
      g++;
   }
   for(int g = 0,  i=0,  p=0 ; g<n*2 ; g++){
    if(g%2 == 0){
        idParDataImpar[g] = array[i];
        i++;
    }
    else{
        idParDataImpar[g] = datasEmSegundos[p];
        p++;
    } 
   }
   quicksort(datasEmSegundos,n);

   
   for(g = 0 ; g<n ; g++){
    array[g] = substitueID(datasEmSegundos[g],idParDataImpar);
    ids[games_sz++] = array[g];
   }
